...
5. **`20-Factions/party/summary.md`** — narrative recap of party trajectory.
6. **`20-Factions/party/leverage.md`** — flat ledger of items, debts, kompromat, and allies-on-call the party currently holds ("what do we have now?"). Faster lookup than scanning summary prose.
7. **`60-Threads/README.md`** — the active-threads roll-up. One table; tells you what's owed by whom and when it comes due ("when does it mature?"). Complements `leverage.md`: threads are time-anchored, leverage is the asset ledger.

---
name: field-ward
description: Officially nonexistent — the unincorporated extramural slum that the dossier names as the city's greatest social vulnerability.
type: ward
---

# Field Ward

**Officially, the Field Ward does not exist.** In reality, it is the city's most dangerous and volatile area: an unincorporated slum in the former killing fields between the northern walls, grown from refugee camps and citizens too poor to pay city taxes. Muddy, lawless, no sewer system, no Watch patrols — only the **City Guard** watching from the walls, intervening only to keep traffic flowing through the gates.

## Index

- [character.md](character.md) — How desperately bad it is, and why the dossier flags it as a powder keg.

## See also

- [../character.md](../character.md) — Disenfranchised Masses; the populist threat.
- [../neighbors.md](../neighbors.md) — for context on Sword Coast peers.
